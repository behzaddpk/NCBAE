...
#Lecture related by adminnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_lecture(request):
    return render(request, 'school/admin_lecture.html')


@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_lecture_view(request, cl):
    lectures = models.Lecture.objects.filter(subject=cl)

    context = {
        'cl': cl,
        'lectures': lectures,
    }
    return render(request, 'school/admin_lecture_view.html', context)
...
@login_required(login_url='teacherlogin')
@user_passes_test(is_teacher)
def teacher_lecture(request):
    return render(request, 'school/teacher_lecture.html')

@login_required(login_url='teacherlogin')
@user_passes_test(is_teacher)
def teacher_upload_lecture(request, cl):
    up_subject = models.Lecture.objects.all()
    user = request.user
    ...
    if request.method == 'POST':
        ...
        lecture = models.Lecture(title=title, subject=subject, video_file=file, uploaded_by=teacher)
        lecture.save()
        return redirect('teacher-lecture')

    context = {
        'subjects': up_subject,
        'cl':cl
    }
    return render(request, 'school/teacher_upload_lecture.html', context)

@login_required(login_url='teacherlogin')
@user_passes_test(is_teacher)
def teacher_lecture_view(request, cl):
    lectures = models.Lecture.objects.filter(subject=cl)
    ...
    return render(request, 'school/teacher_lecture_view.html', context)
...
login_required(login_url='teacherlogin')
@user_passes_test(is_teacher)
def teacher_upload_pdf(request, cl):
    up_subject = models.Lecture.objects.all()
    user = request.user
    teacher = models.TeacherExtra.objects.get(user=user)
    if request.method == 'POST':
        ...
        subject = request.POST.get('subject')
        file = request.FILES.get('file')
        lecture = models.PDFLecture(title=title, subject=subject, PDFFile=file, uploaded_by=teacher)
        lecture.save()
        return redirect('teacher-lecture')
    ...
    return render(request, 'school/teacher_upload_pdf.html', context)
...
def student_lecture_view(request):
    studentdata=models.StudentExtra.objects.all().filter(user_id=request.user.id,status=True)
    lecture = models.Lecture.objects.all().filter(subject=studentdata[0].cl)
    pdflecture = models.PDFLecture.objects.all().filter(subject=studentdata[0].cl)
    ...
    return render(request, 'school/student_lecture_view.html', context)
